...
import pandas as pd
train_df = pd.read_csv ('/content/drive/SyedHaris/train.csv')
display(train_df)
import numpy as np
train_df = pd.DataFrame(np.random.randint(1,110, 80).reshape(-1, 1))
...
from sklearn.model_selection import train_test_split
test=pd.read_csv('/content/drive/SyedHaris/test.csv')
test.head()
clf.predict(t_test)
